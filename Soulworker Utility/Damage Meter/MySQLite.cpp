...
BOOL MySQL::InitMonsterDB() {
	...
	const CHAR* sql = "CREATE TABLE IF NOT EXISTS Monster(Db2 INTEGER, Name_KR TEXT NOT NULL, type INTEGER, PRIMARY KEY(Db2));";
	...
	std::string lang = Language.GetLanguageNameUsedForSQLite();
	std::string sql2 = "SELECT Name_" + lang + ", type From Monster Where Db2 = ?"; //msvc doesnt like defines in + operator of std::string
...
BOOL MySQL::InitMapDB() {
	...
	if (sqlite3_prepare_v2(_db, sql3.c_str(), -1, &_map_stmt_eng, 0) != SQLITE_OK) {
		LogInstance.WriteLog("Error in sqlite3_prepare_v2 2 : %s", sqlite3_errmsg(_db));
