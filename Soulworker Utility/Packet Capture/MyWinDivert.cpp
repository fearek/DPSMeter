...
bool getBytes(std::vector<unsigned char>& buffer, unsigned int size)
{
	...
	while (remaining > 0)
	{
		...
		if (just_read == 0)
		{
			...
			Log::WriteLog(const_cast<LPTSTR>(_T("Module disconnected")));
			return false;
		}
		if (just_read == -1)
		{
			new_socket = INVALID_SOCKET;
			Log::WriteLog(const_cast<LPTSTR>(_T("Error in recv: %d")), WSAGetLastError());
			return false;
		}
		else {
			remaining -= just_read;
...
DWORD MyWinDivert::Init() {
	...
	if (iResult != 0) {
		Log::WriteLog(const_cast<LPTSTR>(_T("WSAStartup failed: %d")), iResult);
		return iResult;
	}
	...
	if (main_socket == INVALID_SOCKET)
	{
		...
		Log::WriteLog(const_cast<LPTSTR>(_T("Socket failed: %d")), error);
		return error;
	};
	int opt = TRUE;
	...
	{
		int error = WSAGetLastError();
		Log::WriteLog(const_cast<LPTSTR>(_T("setsockopt failed: %d")), error);
		return error;
	}
	...
	{
		int error = WSAGetLastError();
		Log::WriteLog(const_cast<LPTSTR>(_T("bind failed: %d")), error);
		return error;
	}
	...
	{
		int error = WSAGetLastError();
		Log::WriteLog(const_cast<LPTSTR>(_T("listen failed: %d")), error);
		return error;
	}
	HANDLE thread = CreateThread(NULL, 0, ReceiveCallback, this, 0, NULL);
	if (thread != NULL)
	{
		Log::WriteLog(const_cast<LPTSTR>(_T("Module Listener Init")));
		return ERROR_SUCCESS;
		}
	else
	{
		DWORD error = GetLastError();
		Log::WriteLog(const_cast<LPTSTR>(_T("CreateThread failed: %d")),error);
		return error;
	}
}
fd_set readfds;
	...
	while (true)
	{
		while (new_socket == INVALID_SOCKET) {
			...
			if (activity == SOCKET_ERROR)
			{
				...
				Log::WriteLog(const_cast<LPTSTR>(_T("select failed: %d")), error);
				exit(-1);
			}
			if (FD_ISSET(main_socket, &readfds))
			{
				...
				{
					int error = WSAGetLastError();
					Log::WriteLog(const_cast<LPTSTR>(_T("Socket in thread failed: %d")), error);
					exit(-1);
				}
				Log::WriteLog(const_cast<LPTSTR>(_T("Module connected")));
			}
			Sleep(100);
		}
