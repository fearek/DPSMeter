...
VOID SWPacketPos::Do() {
	...
	for (int i = 0; i < pos_header->_posPlayerCount; i++) {
		...
		DAMAGEMETER.InsertPlayerMetadata(pos_data->_playerID, utf16, job);
...
VOID SWPacketPos::Debug() {
	...
	for (int i = 0; i < pos_header->_posPlayerCount; i++) {
		...
		//LogInstance.WriteLog(const_cast<LPTSTR>(_T("[DEBUG] [POS PLAYER %d] [ID %08x] [NAME = %s] [NICKMEMSIZE = %d] [JOB = %d]")), pos_data->_playerID, utf16, pos_data->_nickSize, job);
